...
#define LEADER_DATA_PATH \
	"/home/cheoywoo/fixedWing_ws/src/logs/send/UdpData/"
//   "/home/pi/fixedWing_ws/src/leader_date/send/UdpData/"
  
#define LOG_NAME \
	"2020-10-17 19:44:22.log"
//   "2020-10-17 19_44_22.log"
//待处理， 用于多端口接收端头处理
...
fw_msgs::QgcCommands qgcommands_param;
void set_qgcommands_param(const fw_msgs::QgcCommands::ConstPtr& msg){
	qgcommands_param = *msg;
}

fw_msgs::processer UDPexec_param;
void set_UDPexec_param(const fw_msgs::processer::ConstPtr& msg){
	UDPexec_param = *msg;
}
...
{	
	...
	ros::Subscriber setqgcommands_sub = nh.subscribe<fw_msgs::QgcCommands>("/communication/DDT/setqgcommands",1,set_qgcommands_param);
	ros::Subscriber setUDPexec_sub = nh.subscribe<fw_msgs::processer>("/communication/DDT/setudpexec",1,set_UDPexec_param);
	...
	{	
                ...
                switch (case0r_1s)
		{
			...
			case 2:			// 
				...
				if(UDPexec_param.execflag == 0 || UDPexec_param.execflag == -1){
					cout<<"//////////// UDPexecflag ///////////////////////"<<endl;
					cout<<"Not get seng UDPData commands! Now execflag is : ";
					cout<<(int)UDPexec_param.execflag<<endl<<endl;
				}
				// ----------------------------------------------------------------------
				//   进行串口字节发送，并解包 
				// ----------------------------------------------------------------------
				...
				*/
				...
				else if(UDPexec_param.execflag == 1){
					// 获取数据 =======================================
					for (int i = 0; i < 10; i++)
					{ 
				...
				if(QGCflag){
					if(qgcommands_param.tasktype == 0 || qgcommands_param.tasktype == -1){
						cout<<"/////////// qgcommands param ///////////////////////"<<endl;
						cout<<"Not get qgcommands! Now qgccommands is : ";
						cout<<(int)qgcommands_param.tasktype<<endl<<endl;
					}
					else{
						sp_qgcommands.message_id = qgcommands_param.messageid;
						sp_qgcommands.gs_id = qgcommands_param.gsid;
						sp_qgcommands.uav_id = qgcommands_param.uavid;
						sp_qgcommands.vehicleNumber = qgcommands_param.uavnumber;
						sp_qgcommands.taskType = qgcommands_param.tasktype;
						sp_qgcommands.plane_state = qgcommands_param.plane_state;
						sp_qgcommands.plane_group = qgcommands_param.plane_group;
						sp_qgcommands.gapx = qgcommands_param.gapx;
						sp_qgcommands.gapy = qgcommands_param.gapy;
						cout<<"/////////// qgcommands param ///////////////////////"<<endl;
						cout<<"Use Set param!"<<endl;
				...
				{
					unsigned len;
					if(QGCflag && !(qgcommands_param.tasktype == 0 || qgcommands_param.tasktype == -1) ){
					...
					else if(qgcommands_param.tasktype == 0 || qgcommands_param.tasktype == -1){
						cout<<"Not send!"<<endl;
					}
