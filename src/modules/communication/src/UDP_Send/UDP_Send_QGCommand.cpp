...
int main(int argc, char **argv)
{
  ...
  logger_QGC_Command LOGGERQGCCOMMAND("QGCCommand", uavName);  // 保存QGC_COMMAND消息的指令
  ...
  bool messagetype_EF = false;
  Qgc_Commands QC_buf_temp;
  EF execflag_temp;
    ...
    while (argnum < argc)
    {
      ...
      if(argv[argnum] == (string)("-t")){//广播type
        ...
        if(argv[argnum + 1] == (string)("UD")){
            // messagetype = UD;
            messagetype_UD = true;
            cout<<"UD is choiced, is:"<<messagetype_UD<<endl;            
        }
        else if(argv[argnum + 1] == (string)("AC")){
            // messagetype = AC;
            messagetype_AC = true;
            cout<<"AC is choiced, is:"<<messagetype_AC<<endl;
        }
        else if(argv[argnum + 1] == (string)("UH")){
            // messagetype = UH;
            messagetype_UH = true;
            cout<<"UH is choiced, is:"<<messagetype_UH<<endl;
        }
        else if(argv[argnum + 1] == (string)("QC")){
            // messagetype = QC;
            messagetype_QC = true;
            cout<<"QC is choiced, is:"<<messagetype_QC<<endl;
        }
        else if(argv[argnum + 1] == (string)("EF")){
            // messagetype = QC;
            messagetype_EF = true;
            cout<<"EF is choiced, is:"<<messagetype_EF<<endl;
        }
        argnum = argnum + 2;  
      }
        ...
        while((argnum + 1 < argc) && (argv[argnum+1] != (string)("-QCp"))){
          port.push_back(atoi(argv[++argnum]));        // 数组添加
          cout<<"at "<<argnum<<", port : "<<port[port.size()-1]<<endl;
        }
        argnum++;
      }
      else if((messagetype_QC == true) && (argnum + 1 < argc)){

        cout<<"at "<<argnum<<", param : "<<argv[argnum]<<endl;
        QC_buf_temp.gs_id = atoi(argv[++argnum]);
        QC_buf_temp.vehicleNumber = atoi(argv[++argnum]);
        QC_buf_temp.taskType = atoi(argv[++argnum]);
        QC_buf_temp.plane_state = atoi(argv[++argnum]);
        QC_buf_temp.plane_group = atoi(argv[++argnum]);
        QC_buf_temp.gapx = atoi(argv[++argnum]);
        QC_buf_temp.gapy = atoi(argv[++argnum]);
      }
      else if((messagetype_EF == true) && (argnum + 1 < argc)){
        execflag_temp.message_id = 5;
        execflag_temp._flag = atoi(argv[++argnum]);
        cout<<"at "<<argnum<<", param : "<<argv[argnum]<<endl;
      }
  ...
  messagetype_UD = false;
  messagetype_UH = false;
  ...
  /*******************************************************************UDP*************************************************/
  while((ros::ok() && messagetype_QC == false && messagetype_EF == false) || (ros::ok() && (messagetype_QC == true || messagetype_EF == true) && QGCCount < 10)){
    // while(ros::ok() ){
    QGCCount++;
    ...
    if(processer_control.startACK == 1){
      messagetype_AC = true;
      cout<<"QGCommands receive true, send ACK message now"<<endl;
    }
    else if(processer_control.startACK == 2){
      messagetype_AC = true;
      ACKwrong = 1;
      cout<<"QGCommands receive false, send ACK message now"<<endl;
    }
    ...
    if(messagetype_AC == true){ 
      ...
      ACK_buf.judge = processer_control.startACK;
    ...
    if(messagetype_QC == true){
      ...
      QC_buf.gs_id = QC_buf_temp.gs_id;
      QC_buf.uav_id = uavid;
      QC_buf.vehicleNumber = QC_buf_temp.vehicleNumber;
      QC_buf.taskType = QC_buf_temp.taskType;
      QC_buf.plane_state = QC_buf_temp.plane_state;
      QC_buf.plane_group = QC_buf_temp.plane_group;
      QC_buf.gapx = QC_buf_temp.gapx;
      QC_buf.gapy = QC_buf_temp.gapy;
      QC_buf.GPS_time = GPS_time.time_ref.sec + GPS_time.time_ref.nsec * 0.000000001;
      ...
      LOGGERQGCCOMMAND.QGC_Command_gapx.logPrint(QC_buf.gapx);
      LOGGERQGCCOMMAND.QGC_Command_gapy.logPrint(QC_buf.gapy);
      LOGGERQGCCOMMAND.QGC_Command_GPS_time.logPrint(QC_buf.GPS_time);
      LOGGERQGCCOMMAND.QGC_Command_gs_id.logPrint(QC_buf.gs_id);
      LOGGERQGCCOMMAND.QGC_Command_message_id.logPrint(QC_buf.message_id);
      LOGGERQGCCOMMAND.QGC_Command_plane_group.logPrint(QC_buf.plane_group);
      LOGGERQGCCOMMAND.QGC_Command_taskType.logPrint(QC_buf.taskType);
      LOGGERQGCCOMMAND.QGC_Command_uav_id.logPrint(QC_buf.uav_id);
      LOGGERQGCCOMMAND.QGC_Command_parity.logPrint(QC_buf.parity);
      LOGGERQGCCOMMAND.QGC_Command_plane_state.logPrint(QC_buf.plane_state);
    }
    if(messagetype_EF == true){
      printf("///////////////////////////EF//////////////////////////////////////\n");
      execflag.message_id = execflag_temp.message_id;
      execflag._flag = execflag_temp._flag;
      std::cout<<"send message_id    is:"<<execflag.message_id<<std::endl;
      std::cout<<"send execfag    is:"<<execflag._flag<<std::endl;
    ...
    if(messagetype_EF == true){
        cout<<"send EF use broadcast"<<endl;

        cout<<addr_serv.size()<<endl;
        for (int i = 0; i < addr_serv.size(); i++){
          RunSend(sock_fd, execflag, addr_serv[i], len[i]);
        }
        // RunSend(sock_fd, QC_buf, addr_serv[0], len[0]);
      // if(/*addr_serv.size() > 0*/0){
      //   for (int i = 0; i < addr_serv.size(); i++){
