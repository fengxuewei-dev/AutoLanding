...
int main(int argc, char **argv)
{
  ...
  while(ros::ok() && current_state.mode != "ALTCTL" && current_state.mode != "AUTO.MISSION"){
    std::cout << "UDP_Send:  waiting for the mode that is either ALTCTL or AUTO.MISSION" << std::endl;
    ros::spinOnce();
    rate.sleep();
  }
  ...
  while(ros::ok()){
    ...
    // 监控模式 定高模式, mission 模式
    while (!(current_state.mode == "ALTCTL" || current_state.mode == "AUTO.MISSION"))
    {
      std::cout << "UDP_Send:  waiting for the mode that is either ALTCTL or AUTO.MISSION!" << std::endl;
      ros::spinOnce();
      rate.sleep();
  ...
  #ifndef    _USE_THREE_DIMENSION_VF_
      UDP_buf.altitude_z = current_leaderPose.pose.position.z;
  #else
      UDP_buf.altitude_z = 100;
  #endif

      UDP_buf.heading = current_Leader_state.heading;
      
      // 改动
      UDP_buf.velocity = current_Leader_state.groundspeed;
